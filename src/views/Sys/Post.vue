<template>
	<div>
	  <div style="margin-bottom: 10px; text-align: left;">
	  	...
	  	<Xyj-button type="primary" size="mini" @click="seePostUser" :label="$t('action.viewemployees')" text="查看员工"></Xyj-button>
	  </div>
    <el-input
      ...
      <el-form :model="formData" :rules="dataRule" ref="dataForm" label-width="80px" size="small" style="text-align:left;">
        <el-form-item label="上级名称" prop="name">
          <el-input v-model="formData.name" placeholder="名称" disabled></el-input>
        </el-form-item>
        <el-form-item label="编码" prop="code">
          <el-input v-model="formData.code" placeholder="编码"></el-input>
        </el-form-item>
        <el-form-item label="岗位名称" prop="postName">
          <el-input v-model="formData.postName" placeholder="岗位名称"></el-input>
        </el-form-item>
      ...
      getQueryRole(){
    		...
    		if(arr.length>0){
	    		...
	    		this.$nextTick(()=>{ 
	    			...
	    			for(let key in this.formData){
	    				if(key === 'name') this.formData.name = this.$refs.tree.getCheckedNodes()[0].postName
    	...
    	edit(callback){
    		...
    		if(arr.length>0){
	    		...
	    		for(let key in this.formData){	
	    			if(key === 'name') this.formData[key] = array.length>0 ? array[0].postName : data[0].postName.split('|')[0]
	    			else if(key === 'cid') this.formData[key] = array.length>0 ? array[0].id : data[0].id
	    			else if(key === 'postName') this.formData[key] = data[0].postName.split('|')[0]
    	...
    	seePostUser(callback){
    		...
    		if(arr.length>0){
		        ...
		        hostUrl: this.global.baseUrl,
		        method: "get",
		        url:"/sysPostUser/findUserById?id="+this.$refs.tree.getCheckedKeys()[0],
		      })
					.then(res => {
						...
						function formatter(data){
							data.forEach(element => {
								if(element.children && element.children.length>0){
									...
									element.postName = element.postName + ' | ' + element.code	
									return true;
								}	
								element.postName = element.postName + ' | ' + element.code
							})
