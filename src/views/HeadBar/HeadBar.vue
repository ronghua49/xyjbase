  ...
  <div class="headbar" :style="{'background':themeColor}" 
    ...
    <!--<span class="navbar">
      <el-menu :default-active="activeIndex" class="el-menu-demo" 
        ...
        <el-menu-item index="2" @click="openWindow('https://gitee.com/liuge1988/XYJ')">{{$t("common.projectRepo")}}</el-menu-item>
        <el-menu-item index="3" @click="openWindow('https://gitee.com/liuge1988/XYJ/wikis/Home')">{{$t("common.doc")}}</el-menu-item>
    ...
    <span class="toolbar">
      <el-menu class="el-menu-demo" :background-color="themeColor" :text-color="themeColor" :active-text-color="themeColor" mode="horizontal">
      	<el-menu-item index="6">
           <!--聊天--> 
            <el-badge :value="value" type="primary" class="item" :hidden="hidden">
						  <el-button class="fa fa-comments-o" type="text" @click="chat = !chat" style="color: #fff;font-size: 20px;"></el-button>
						</el-badge>
        </el-menu-item>
    ...
    <vue-chart v-show="chat" ref="vuechart"></vue-chart>
  </div>
</template>
...
import VueChart from "@/components/vueChart/index"
export default {
  components:{
        Hamburger,
        ThemePicker,
  ...
  methods: {
    ...
    open(){
    	this.$refs.vuechart.open()
    }
  },
  mounted() {
    this.sysName = "xyj Platform"
    var user = sessionStorage.getItem("user")
    if (user) {
      this.user.name = user
      this.user.avatar = require("@/assets/my1.png")
    }
    this.$api.im.HistoryQuery({
      ...
      if(res.data.rows.length>0) {this.value = res.data.rows.length;this.hidden = true;}
    }).catch((error)=>{
