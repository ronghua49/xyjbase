  ...
  <div class="el-data-table xyj-table" @click.ctrl="showTable">
    <el-row class="xyj-tooltip" type="flex" align="middle" justify="space-between" id="aaa">
      ...
      <el-row  type="flex" align="middle" justify="end">
        ...
        <el-form >
          <el-form-item >  
          	<Xyj-button style="padding: 7px;" v-if="hasExport" type="primary" :disabled="disabledExport"
                      @click="onDefaultExport" :tableName="tableName" :label="$t('action.export')" text="导出" />
            ...
            <Xyj-button style="padding: 7px;" v-show="hasSelect && hasDelete" type="danger" :label="$t('action.batchDelete')" text="批量删除"
                       @click="onDefaultDelete($event,selected,'many')" :tableName="tableName"
                       :disabled="single ? (!selected.length || selected.length > 1) : !selected.length || disabledNew" />
          </el-form-item>
        </el-form> 
	      <el-row  type="flex" align="middle" justify="end" v-if="hasSearch ? getQuickSearch.length > 0 || !!$slots.search : hasSearch" >
	        <!--搜索字段-->
	        <el-form-renderer class="serach-style" v-if="getQuickSearch.length > 0 || !!$slots.search" inline :content="getQuickSearch" ref="searchForm">
	          <!--@slot 额外的搜索内容, 当searchForm不满足需求时可以使用-->
	          <slot name="search"></slot>
	        </el-form-renderer>
	        <el-form>
	          <el-form-item>
	            <el-button  type="primary" @click="search" size="mini" icon="el-icon-search" circle title="查询"></el-button>
    ...
    <el-table ref="table" v-bind="getTableAttrs" :data="data" 
      ...
      <template v-else>
        ...
        <template v-for="item in getTableColumns">
          ...
          <el-table-column v-else-if="item.isAllowSearch == 1" :key=item.id v-bind="item" sortable = "custom"></el-table-column>
        </template>
      </template>
      <!--默认操作列-->
      <el-table-column label="操作" v-if="hasOperation" v-bind="operationAttrs" fixed="left" align="center">
        <template slot-scope="scope">
          <el-row type="flex" justify="center">
            ...
            <Xyj-button style="padding: 7px;" v-if="hasEdit" :label="$t('action.edit')" text="编辑" :disabled="scope.row.isShow ? scope.row.isShow : disabledEdit || disabledNew" 
            ...
            <Xyj-button style="padding: 7px;"
                       ...
                       :label="$t(btn.label)"
                       v-bind="btn" @click="onCustomButtonsClick($event,btn.atClick, scope.row)" :key="i" size="mini"
                       :loading="customButtonsLoading" :text="btn.text">{{btn.text}}</Xyj-button>
 ...
 //// Vue.prototype.$confirm = MessageBox.confirm
  ...
  let relationship = {
    ...
    $options:[
      ...
      {label: '等于忽略大小写',value: 'EQUAL_IGNORE_CASE'},
      {label: '小于',value: 'LESS'},
      {label: '小于等于',value: 'LESS_EQUAL'},
      {label: '大于',value: 'GREAT'},
      {label: '大于等于',value: 'GREAT_EQUAL'},
      {label: '不等于',value: 'NOT_EQUAL'},
      {label: '相似',value: 'LIKE'},
      {label: '左相似',value: 'LEFT_LIKE'},
      {label: '右相似',value: 'RIGHT_LIKE'},
     ...
     */  
     ...
     //  排序时的查询条件
      ...
      /** 
			 ...
			 * @param {Array} array
			 * @return {Boolean} 返回值 
			 * 表格查询字段下拉框需要展示的字段，传过来的是个字段名，例如 tablename，
			 * 如果有这个数组会显示这个数组中的字段没有显示全部表格中的字段
			 * @author 张俊娜 
			 */
      searchList:{
      	type: Array
    ...
    methods: {
      ...
      getQuery(findWhere){
        ...
        if(!findWhere){
          ...
          //  Object.assign IE不支持, 所以后面Object.keys的保守其实是没有必要的。。。
      ...
      /**
       *宋建国：2019/11/7 12:11
       *描述：true：空条件查询，false:空条件不查询
       *版本：1.0
       */
      setNotCondition(value){
      ...
      getList(findWhere){
        ...
        if(!this.notCondition && query.querys.length<=0){
          return;
        }
        if(this.codeVaule){
        ...
        axios(
          ...
          /**
          * 请求返回, 数据更新后触发, 返回(data, resp) data是渲染table的 数据, resp是请求返回的完整response
          * @event update
          */
...
/*        if(this.routerMode && shouldStoreQuery > 0) {
      ...
      onDefaultExport(cab){
        let then = this;
        //得到查询条件
        let data = then.getQuery();
        let tmpExport = Object.assign({},then.bExport);
        /**
         *宋建国：2019/9/7 17:16
         *描述：导出网址默认值处理
         *版本：1.0
         */
        if (then.$tu.isBlank(tmpExport.url)){
          tmpExport.url = then.bQuery.hostUrl + '/'+ then.bQuery.url;
          tmpExport.hostUrl = then.bQuery.hostUrl;
          if(then.$tu.isBlank(tmpExport.url)){
            then.$message.error('导出数据网址不能为空');
            cab()
            return;
          }
        }
         ...
         *版本：1.0
         */
        if (then.$tu.isBlank(tmpExport.tableName)){
          tmpExport.tableName = then.tableName;
          if(then.$tu.isBlank(tmpExport.tableName)){
            then.$message.error('数据表名不能为空.');
            cab()
            return;
          }
        }
        data["selfDate"] = Object.assign({},{},tmpExport);
        /**
         *宋建国：2019/9/7 17:17
         *描述：调用接口导出数据
         *版本：1.0
         */
        then.$api.common.exportExcel(
          {data}
        ).then((res) => {
          //处理下载文件
          then.$tu.downloadfile(res);
          cab()
        }).catch((err) => {
          ...
          this.beforeConfirm(data,this.isNew).then(resp => {
            ...
            if(this[condiction] && this[customMethod]){
              this[customMethod](data,this.row).then(resp => {
                ...
                if(this.onAfterSuccessNew && this.isNew){
                 ...
                 *版本：1.0
                 */
                if(this.onAfterSuccessEdit && this.isEdit){
                  this.onAfterSuccessEdit(data,this.row);
                }
              }).catch(e => {
            ...
            axios(
              ...
              //onNew事件执行成功之后的回调方法 宋建国
              if(this.onAfterSuccessNew && this.isNew){
                this.onAfterSuccessNew(res.data,this.row);
              }
              /**
               *宋建国：2019/11/7 17:14
               *描述：编辑事件成功之后的回调方法
               ...
               */
              if(this.onAfterSuccessEdit && this.isEdit){
                this.onAfterSuccessEdit(res.data,this.row);
              }
              if(url.clear){
                this.$refs[dialogForm].resetFields();
              }
              ...
              if(this.onAfterFailNew && this.isNew){
                this.onAfterFailNew(data,err.message);
              }
              if(this.onAfterFailEdit && this.isEdit){
                this.onAfterFailEdit(data,err.message);
              }
              this.confirmLoading = false;
            })
          }).catch(e => {
          ...
          beforeClose: (action,instance,done) => {
            if(action == 'confirm'){
              ...
              if(!then.bDeleteUrl.hostUrl || !then.bDeleteUrl.url){
                this.$message.error("服务器网址为空，不能发送请求");
                instance.confirmButtonLoading = false
                cab()
                return;
              }
      ...
      onCustomButtonsClick(cb,fn, parameter) {
        if (!fn) return
          ...
          summaryMethod: function (objData) {
            ...
            tmpcolumns.forEach((column,index) => {
              if(column.property){
                ...
                let tmpValue =then.summaryData[fieldname];
      ...
      axios(data).then((res) => {
        if (this.tableColumns.length === 0) {
          ...
          this.tableColumns.forEach((column,index) =>{
            ...
            if(this.formatters.hasOwnProperty(column.prop) || column.children){
							column.children
              ?column.children.forEach(data => {
                if(this.formatters.hasOwnProperty(data.prop)) data.formatter = this.formatters[data.prop];
              })
        ...
        this.allTableColumns.forEach((column, index) => {
          ...
          if(this.searchList && this.searchList.length>0){
          	this.searchList.forEach(response => {
	          	if(column.prop == response && (column.isAllowSearch == 1 || column.isAllowSearch == 2)) {
	          		this.fieldSelectValue.push({key: column.prop, label: column.label, value: column.prop});
	          	}
	          })
          }else{
          	if(column.isAllowSearch == 1 || column.isAllowSearch == 2){
          		this.fieldSelectValue.push({key: column.prop, label: column.label, value: column.prop});
          	} 	
          }
//        this.fieldSelectValue.push({key: column.prop, label: column.label, value: column.prop});
        ...
        if(this.getQuickSearchs[0]){
          if(this.getQuickSearchs[0].$items[0].$options.length>0){
            this.getQuickSearchs[0].$items[0].$default = this.fieldSelectValue.length>0?this.fieldSelectValue[0].label:this.getQuickSearchs[0].$items[0].$default
          }
          this.getQuickSearchs[0].$items[0].$default = this.fieldSelectValue.length>0?this.fieldSelectValue[0].label:''
        }
        this.$nextTick(() => {
          /**
           *宋建国：2019/11/7 12:07
           *描述：设置空条件查询开关变量，true:空条件查询，false:空条件不查询
           *版本：1.0
           */
          this.notCondition = this.bQuery.notCondition;
