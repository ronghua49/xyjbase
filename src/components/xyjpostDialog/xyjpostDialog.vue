			...
			<el-row type="flex" justify="space-between">
				...
				<el-col :span="6" style="border-left: 1px solid #ccc;box-sizing: border-box;padding-left: 10px;height: 400px;overflow-y: auto;">
					...
					<el-tooltip effect="dark" content="双击移除当前已选择的岗位" placement="bottom-start" v-for="(item,index) in chooseList" :key="item.id">
						<span style="display: block;cursor: pointer;" @dblclick="removePost(item)">{{ (index+1)+'、'+item.postName }} <span v-html="radio == item.id ? '(主岗位)' : ''"></span></span>
					</el-tooltip>
				</el-col>
			</el-row>
	...
	export default {
		...
		methods: {
			...
			choose(val, data) {
				...
				//				? this.$refs.tree.getCheckedNodes().indexOf(data) != -1 
				//				? this.$refs.tree.setCheckedNodes(this.$refs.tree.getCheckedNodes())
				//				: this.$refs.tree.setCheckedNodes(this.$refs.tree.getCheckedNodes().concat([data])) 
